...
class Triplet:
    def __init__(self, kmer_len):
        ...
        with tf.variable_scope('input'):
            ...
            self.samples = tf.placeholder(tf.float32, shape=[None, kmer_len, seq_dim], name='samples')

        with tf.variable_scope('embedding') as scope:
            self.embeddings = self.__embedding_network(self.samples, kmer_len)

        with tf.variable_scope('loss'):
            self.loss = tf.contrib.losses.metric_learning.triplet_semihard_loss(self.labels, self.embeddings, margin=1.0)

            # labels: 2 - D Tensor of labels of shape[batch size, 1]
            # embeddings: 2 - D Tensor of embeddings of shape [batch size, embedding dimension].
...
batch_indices = np.random.randint(0, len(X_test) - 1, batch_size)
batch_samples = np.zeros((batch_size, len(X_test[0]), seq_dim))
batch_labels = np.zeros((batch_size, 1), dtype=np.int32)
for i in range(batch_size):
    batch_samples[[i], :, :] = [seq2binary(X_test[batch_indices[i]])]
    batch_labels[i] = batch_indices[i]

print("batch samples shape: {}".format(batch_samples.shape))
...
# print(batch_labels)
# print(array_ops.shape(batch_labels))
# print(array_ops.shape(batch_labels).shape)

# ValueError: Cannot feed value of shape (32,) for Tensor 'input/labels:0', which has shape '(?, 1)'
...
with tf.Session() as sess:
    ...
    for i in range(iterations):
        ...
        if i % logging_frequency == 0:
            loss = sess.run([triplet.loss], feed_dict={triplet.labels: batch_labels, triplet.samples: batch_samples})
            print('step %d, training loss %g' % (i, loss))
        train_step.run(feed_dict={triplet.labels: batch_labels, triplet.samples: batch_samples})
...
# with self-defined Triplet loss:
